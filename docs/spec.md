...
where `sum` is whatever the implementation's language or platform makes natural
for reducing an array — `numpy.sum` in Python, `Base.sum` in Julia,
`=SUM(range)` in a spreadsheet. No accumulation order is mandated.

Floating-point addition is not associative, so implementations will not agree to
the last bit, and an implementation whose reduction depends on the host — vector
width, thread count — will not always agree with itself. Each case declares a
tolerance (§ 4.1) wide enough to cover that and narrow enough to catch a real
error. Implementations are compared against the golden value, never against
each other.
...
### 4.1 Golden values and tolerance
